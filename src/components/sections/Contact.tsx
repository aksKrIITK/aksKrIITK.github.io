...
function ContactInfoCard({ icon, label, value, href }: { icon: string; label: string; value: string; href?: string }) {
  ...
  const inner = (
    <div {...hoverProps} className={`flex gap-3.5 items-center p-3.5 px-4.5 rounded-xl border transition-all duration-200  ${hovered ? 'bg-bg4 border-accent/30 shadow-[0_4px_14px_rgba(250,204,21,0.1)]' : 'bg-bg3 border-white/10'
...
export function Contact() {
  ...
  const inputClasses = "w-full p-3.5 px-4.5 rounded-lg bg-bg3 border border-white/10 text-textLight font-poppins text-[15px] outline-none transition-colors duration-200 focus:border-accent focus:bg-bg4";
    ...
    <section id="contact" className="py-[100px] px-7 bg-bg relative overflow-hidden">
      ...
      <div className="max-w-[900px] mx-auto relative z-10">
        <div className="text-center mb-14 flex flex-col items-center">
          ...
          <h2 className="font-sora font-extrabold text-[clamp(32px,4vw,52px)] text-textLight m-0 mb-3">
            Work With Me
          </h2>
          <p className="font-poppins text-[17px] text-muted">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1.6fr] gap-10">
          ...
          {/* Form */}
          <div className="bg-bg3  rounded-2xl p-9 border border-white/10">
            ...
            <button className="w-full p-3.5 rounded-full bg-accent text-black font-poppins font-bold text-base border-none cursor-pointer tracking-[0.03em] hover:opacity-90 transition-opacity shadow-[0_4px_14px_rgba(250,204,21,0.4)] uppercase">
              Send Message →
            </button>
